...
def norm_xcorr2d(patch, template):
    ...
    for row in patch:
        for ele in row:
            patch_mean = patch_mean + ele

    patch_mean = patch_mean / (heigth * width)


    template_mean = 0
    for row in patch:
        for ele in row:
            template_mean = template_mean + ele

    template_mean = template_mean / (heigth * width)

    patch_udt = patch - patch_mean
    template_udt = template - template_mean

    temp = utils.elementwise_mul(patch_udt,template_udt)
    for row in temp:
        for ele in row:
            numerator = numerator + ele

    temp2 = utils.elementwise_mul(template_udt,template_udt)
    temp3 = utils.elementwise_mul(patch_udt,patch_udt)
    sum1 = 0
    sum2 = 0
    for row in temp2:
        for ele in row:
...
def match(img, template):
    ...
    for row in range(0,img_heigth - temp_height):
        Y_min = 0
        Y_max = Y_min + temp_width
        for col in range(0,img_width - temp_width):

            check = utils.crop(img, X_min, X_max, Y_min, Y_max)
            ans = norm_xcorr2d(check,template)
            if (ans > max_value):
                max_value = ans
                x = X_min
                y = Y_min 
    ...
    #check = utils.crop(img, 17, 17+24, 129, 129+26)
    #ans = norm_xcorr2d(check,template)
    #print (ans.shape)
